...
function hackyOffset(dtf, date) {
  ...
  const parsed = /(\d+)\/(\d+)\/(\d+) (AD|BC),? (\d+):(\d+):(\d+)/.exec(
    formatted,
  );
...
function convert(matrix, fromMap, fromUnit, toMap, toUnit) {
  ...
  const added = !sameSign && toMap[toUnit] !== 0 && Math.abs(raw) <= 1
    ? antiTrunc(raw)
    : Math.trunc(raw);
  toMap[toUnit] += added;
